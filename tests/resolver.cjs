// Maps relative ESM `.js` import specifiers to the corresponding `.ts` source file
// (the plugin uses NodeNext-style `.js` specifiers that point at `.ts` sources during tests).
const fs = require('node:fs');
const path = require('node:path');

module.exports = (request, options) => {
  const isRelativeJavaScriptImport =
    (request.startsWith('./') || request.startsWith('../')) && request.endsWith('.js');

...
export class ChatSurfaceService {
  ...
  private createEventEmitter(sink: ChatSurfaceEventSink): AgentEventEmitter {
    return async (event) => {
      ...
      if (event.type === "error") {
        // Full error detail is logged server-side (see runAndPersistAgentResponse);
        // external chat users receive a generic message to avoid leaking internals.
        await sink.emit({
          type: "error",
          message: "Agent response failed. Check server logs for details.",
        });
      }
    };
  }
  ...
  private async handleAudioMessage(
    ...
    try {
      ...
      await sink.emit({
        type: "error",
        message: "Speech synthesis failed. Check server logs for details.",
      });
    }
  }
